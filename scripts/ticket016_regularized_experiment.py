"""
TICKET-016正則化版 67特徴量 + BPM Stratified戦略実験
exp09_1の最高LB性能(26.38534)パラメータを67特徴量版に適用
"""
...
def train_lightgbm_fold_regularized(X_train, y_train, X_val, y_val):
    """単一フォールドでLightGBM訓練（exp09_1正則化パラメータ）"""
    train_data = lgb.Dataset(X_train, label=y_train)
    val_data = lgb.Dataset(X_val, label=y_val, reference=train_data)

    # exp09_1最高性能パラメータ適用（強化正則化）
    lgb_params = {
        ...
        'bagging_freq': 1,
        'reg_alpha': 2.0,      # L1正則化（exp09_1設定）
        'reg_lambda': 2.0,     # L2正則化（exp09_1設定）
...
def run_regularized_67_experiment():
    """67特徴量正則化版BPM Stratified実験実行"""
    logger.info("TICKET-016正則化版 67特徴量 + BPM Stratified実験開始...")
    ...
    logger.info("BPM Stratified KFold正則化訓練・推論開始...")
    ...
    for fold, (train_idx, val_idx) in enumerate(tqdm(skf.split(X_train, stratify_labels), total=n_folds)):
        ...
        # 正則化訓練
        model = train_lightgbm_fold_regularized(X_fold_train, y_fold_train, X_fold_val, y_fold_val)
        models.append(model)
    ...
    submission_path = config.processed_data_dir / f"submission_ticket016_regularized_67_{timestamp}.csv"
    ...
    # 実験記録
    experiment_record = {
        "experiment_name": "exp17_ticket016_regularized_67",
        "timestamp": timestamp,
        "cv_strategy": "bpm_stratified",
        "model_type": "lightgbm_regularized",
        ...
        "target_lb": best_lb,
        "expected_lb_performance": f"exp09_1正則化パラメータ適用による性能向上期待"
    }

    record_path = config.processed_data_dir / f"experiment_record_ticket016_regularized_{timestamp}.json"
...
def submit_to_kaggle(submission_path: Path, cv_score: float, improvement: float):
    ...
    message = f"TICKET-016 Regularized 67 Features + BPM Stratified - CV: {cv_score:.6f} (improvement: {improvement:+.6f}) Target: 26.38534"
...
if __name__ == "__main__":
    logger.info("TICKET-016正則化版67特徴量実験開始")

    submission_path, cv_score, cv_std, improvement = run_regularized_67_experiment()

    logger.success("実験完了！")
    logger.info(f"CV RMSE: {cv_score:.6f} ± {cv_std:.6f}")
    logger.info(f"67特徴量ベースからの改善: {improvement:+.6f}")

    submit_to_kaggle(submission_path, cv_score, improvement)

    logger.info("次のステップ:")
    logger.info("1. LBスコア確認（目標: 26.38534超越）")
    logger.info("2. 正則化効果の検証")
    logger.info("3. 二元アンサンブルへの適用検討")